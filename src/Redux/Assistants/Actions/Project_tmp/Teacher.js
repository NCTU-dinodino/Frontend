...
export const setAddStatus = (post_item) => dispatch => {
  axios.post('/_api/assistants/project/setAddStatus', post_item).then(res => {
...
export const setFirstSecond = (post_item) => dispatch => {
  axios.patch('/_api/assistants/project/setFirstSecond', post_item).then(res => {
    if (res.data.signal === 1) {
...
export const deteleResearch = (post_item) => dispatch => {
  axios.post('/_api/assistants/project/delete', post_item).then(res => {
    if (res.data.signal === 1) {
...
export const downloadCsv = req => dispatch => {
  ...
  axios.post('/_api/assistants/project/professorList/download', req).then(res => {
    let data = res.data
    let csvArr = []
