...
export const fetchCheck = (payload) => dispatch => {
  axios.all([
    axios.post('/_api/assistants/project/professorList', {
      ...payload,
      first_second: "1"
    }),
    axios.post('/_api/assistants/project/professorList', {
...
export const agreeCheck = (payload) => dispatch => {
  axios.all([
    axios.post('/_api/assistants/project/setAddStatus', payload),
    axios.post('/_api/assistants/project/setFirstSecond', payload)
  ]).then(axios.spread((first_res, second_res) => {
...
export const rejectCheck = (payload) => dispatch => {
  axios.post('/_api/assistants/project/delete', payload).then( res => {
    dispatch(check_delete(payload.student_id))
    window.confirm("退選成功")
  }).catch(err => 
